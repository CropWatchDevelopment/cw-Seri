...
bool LORA_Uart_Transmit(uint8_t const * const tx_data_p, uint16_t tx_length, UartJob_t** status_p)
{
    ...
    bool startedOk = Uart_StartTransmission(LORAWAN_UART, &huart2, tx_data_p, tx_length);
    return startedOk;
}
...
bool LORA_Uart_Receive(uint8_t* rx_data_p, uint16_t rx_length, UartJob_t** status_p)
{
    ...
    bool rx_started = Uart_StartReceiving(LORAWAN_UART, &huart2, rx_data_p, rx_length);
    return rx_started;
}
