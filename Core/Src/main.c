...
static volatile uint16_t wakeup_counter = 0u; // incremented in ISR
static uint16_t wakes_accum = 0u;             // main-loop accumulator
...
// Set Dev EUI, App Key, and JOIN KEY
  ...
  while (1)
  {
      ...
      /* USER CODE BEGIN 3 */
      uint16_t calculated_temp = 0u, calculated_hum = 0u;
...
void Error_Handler(void)
{
    ...
    static uint32_t low_ticks = 0uL;
    static uint32_t high_ticks = 0uL;
#endif
...
#if defined (DEBUG_PHASE)
  while (1) {
      if (++low_ticks == 0uL) { // i.e. an overflow occurred
          high_ticks++;
      }
  }
#else
